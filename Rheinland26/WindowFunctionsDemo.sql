...
DROP TABLE IF EXISTS [#InvTable] 

CREATE TABLE [#InvTable] (
  InvName VARCHAR(20) NOT NULL, StartDate DATE NOT NULL, ProfitPct DECIMAL(5,1) NOT NULL
  CONSTRAINT PK PRIMARY KEY(InvName, StartDate)
)

INSERT INTO [#InvTable] VALUES
  ('Conservative', '2020-01-01', 2.0), ('Conservative', '2021-01-01', 1.0), ('Conservative', '2022-01-01', 2.5), ('Conservative', '2023-01-01', 1.0), ('Conservative', '2024-01-01', 2.5),
  ('Risky', '2020-01-01', 4.0), ('Risky', '2021-01-01', -3.0), ('Risky', '2022-01-01', 2.0), ('Risky', '2023-01-01', -2.0), ('Risky', '2024-01-01', 5.0), ('Sample', '2023-01-01', 100.0), ('Sample', '2024-01-01', -50.0)

SELECT * FROM [#InvTable] 
WHERE InvName = 'Sample'

SELECT InvName, SUM(ProfitPct) SumInterest
FROM #InvTable
GROUP BY InvName

SELECT InvName, (PRODUCT(1 + ProfitPct / 100.0) - 1) * 100.0 TotalInterest
FROM #InvTable
GROUP BY InvName

SELECT InvName, StartDate, (PRODUCT(1 + ProfitPct/100.0) OVER(PARTITION BY InvName ORDER BY StartDate ROWS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) - 1 ) *100.0 AS FutureInterest
FROM #InvTable
ORDER BY StartDate, InvName
...
DROP TABLE IF EXISTS [#SomeOrders] 
...
INSERT INTO [#SomeDates](TheDate) VALUES 
  ('2025-09-22'),
  ('2025-09-23'),
  ('2025-09-25'),
  ('2025-09-26'),
...
SELECT DATEADD(DAY, 1, currentDate) AS GapStart, DATEADD(DAY, -1, nextDate) AS GapEnd, DATEDIFF(DAY, currentDate, nextDate) - 1 AS GapSize
FROM CTE
