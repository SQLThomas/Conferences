...
neder.raw <- shapefile('data/TopGrenzen-prov-actueel')
neder <- spTransform(neder.raw, CRS("+proj=longlat +datum=WGS84"))
plot(neder)
prov_nl <- read.csv2('data/nl.csv')
prov_nl
with(prov_nl, points(Lon, Lat, pch = 21, bg = 'blue'))
title(main = 'Nederlandse provincies')
with(prov_nl, text(Lon + 0.08, Lat, Capital, pos = 1, cex = 0.8, col = 'darkblue'))
