...
-- ©2019 Thomas HŸtter, this script is provided as-is for demo and educational use only,
...
SELECT id, NAME_1, VARNAME_1, geom FROM gadm36_SWE_0

SELECT id, NAME_1, VARNAME_1, geom FROM gadm36_SWE_1

SELECT id, NAME_1, NAME_2, geom FROM gadm36_SWE_2

SELECT id, NAME_2 AS Name_, geom FROM gadm36_SWE_2 WHERE NAME_2 = 'Stockholm'
UNION ALL
SELECT id, NAME_1 AS Name_, geom FROM gadm36_SWE_1 where NAME_1 = 'Stockholm'
UNION ALL
SELECT id, NAME_1 AS Name_, geom FROM gadm36_SWE_0


-- Applications
DECLARE @ww geography
SELECT @ww = geom FROM gadm36_SWE_2 WHERE NAME_2 = 'Vallentuna'
SELECT id, 'Big Tuna' AS Name_, geom.STUnion(@ww) AS geom FROM gadm36_SWE_2 WHERE NAME_2 = 'Sigtuna'
UNION ALL
SELECT id, NAME_1 AS Name_, geom FROM gadm36_SWE_1 where NAME_1 = 'Stockholm'
...
SELECT @m = geom FROM gadm36_SWE_2 WHERE NAME_2 LIKE 'Eskilstuna'
SELECT @n = geom FROM gadm36_SWE_2 WHERE NAME_2 = 'Stockholm'
SELECT @l = @m.ShortestLineTo(@n)
SELECT @m.STDistance(@n) / 1000 AS "Dist (km)"

SELECT 'line' AS Name_, @l
UNION ALL
SELECT NAME_2 AS Name_, geom FROM gadm36_SWE_2 WHERE NAME_2 = 'Stockholm'
UNION ALL
SELECT NAME_2 AS Name_, geom FROM gadm36_SWE_2 WHERE NAME_2 = 'Eskilstuna'

UNION ALL
SELECT NAME_2 AS Name_, geom FROM gadm36_SWE_2 WHERE geom.STIntersects(@l) = 1


-- Show neighbouring cities
DECLARE @e geography
SELECT @e = geom FROM gadm36_SWE_2 WHERE NAME_2 = 'Eskilstuna'
SELECT id, NAME_1 AS Name_, geom FROM gadm36_SWE_1 WHERE NAME_1 = 'Södermanland'
UNION ALL
SELECT id, NAME_2 AS Name_, geom FROM gadm36_SWE_2
...
SELECT GEN, geom FROM Spatial1.dbo._sta where id = 2
UNION ALL
SELECT RG, geom.BufferWithCurves(60000) AS geom FROM Spatial1.dbo.pass_de

DECLARE @pass_geom geography
SELECT @pass_geom = geography::UnionAggregate(geom.BufferWithCurves(60000)) FROM Spatial1.dbo.pass_de
