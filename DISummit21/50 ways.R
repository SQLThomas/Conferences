...
ggplot(mpg, aes(cty, hwy)) + 
  ...
  labs(title = 'Scatterplot with 234 data points', 
...
de <- data.frame(
  parties = factor(c("Fraktionslos", "AfD", "FDP", "CDU", "CSU", "Gruene", "SPD", "Linke"),
                   levels = c("Fraktionslos", "AfD", "FDP", "CDU", "CSU", "Gruene", "SPD", "Linke")),
  seats   = c(8, 88, 80, 200, 45, 67, 152, 69),
  colors  = c("grey", "lightblue", "yellow", "black", "blue", "green", "red","purple"),
  stringsAsFactors = FALSE)

se <- data.frame(
  grupper = factor(c("KD", "M", "C", "L", "SD", "MP", "S", "V"),
                   levels = c("KD", "M", "C", "L", "SD", "MP", "S", "V")),
  parties = c("Kristdemokraterna", "Moderaterna", "Centerpartiet", "Liberalerna", "Sverigedemokraterna", "Miljöpartiet", "Socialdemokraterna", "Vänsterpartiet"),
  seats   = c(22, 70, 31, 20, 62, 16, 100, 28),
  colors  = c("darkblue", "blue", "darkgreen", "lightblue", "yellow", "lightgreen", "red", "red4"),
  stringsAsFactors = FALSE)

se
ggplot(se) + 
  geom_parliament(aes(seats = seats, fill = grupper), color = "black") + 
  scale_fill_manual(values = se$colors, labels = se$parties) +
  coord_fixed() + labs(title = 'Sveriges riksdag', caption = 'Source: Riksdagen 09/21') +
  theme_void() + theme(plot.title = element_text(hjust = 0.5))

ggplot(se) + 
  geom_arcbar(aes(shares = seats, fill = parties, r0 = 5, r1 = 10), sep = 0.05) + 
  scale_fill_manual(values = se$colors) +
  coord_fixed() + labs(title = 'Sveriges riksdag', caption = 'Source: Riksdagen 09/21') +
  theme_void() + theme(plot.title = element_text(hjust = 0.5))
...
p <- ggplot(df, aes(x, y)) + geom_point() + 
  labs(title = 'Scatterplot', subtitle = 'Random data')
p
...
# load('Data/klevemap.Rdata')# map.hull <- suppressMessages(qmap("Hull", zoom=13, source = "google", maptype="roadmap"))
map.kleve + 
