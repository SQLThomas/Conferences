...
-- ©2018 Thomas HŸtter, this script is provided as-is for demo and educational use only,
...
SELECT id, NAME_0, NAME_LOCAL, geom FROM POL_adm0

SELECT id, NAME_1, VARNAME_1, geom FROM POL_adm1

SELECT id, NAME_1, NAME_2, geom FROM POL_adm2 WHERE NAME_2 = 'Warsaw'

SELECT id, NAME_2 AS Name_, geom FROM POL_adm2 where NAME_2 = 'Warsaw'
UNION ALL
SELECT id, NAME_1 AS Name_, geom FROM POL_adm1 where NAME_1 = 'Masovian'
UNION ALL
SELECT id, NAME_0 AS Name_, geom FROM POL_adm0

SELECT id, NAME_2 AS Name_, geom FROM POL_adm2 where NAME_2 = 'Radom'


-- Applications
DECLARE @ww geography
SELECT @ww = geom FROM POL_adm2 WHERE NAME_2 = 'Warsaw West'
SELECT id, 'Greater Warsaw' AS Name_, geom.STUnion(@ww) AS geom FROM POL_adm2 WHERE NAME_2 = 'Warsaw'
UNION ALL
SELECT id, NAME_1 AS Name_, geom FROM POL_adm1 where NAME_1 = 'Masovian'
...
SELECT @m = geom FROM POL_adm2 WHERE NAME_2 LIKE 'Radom City'
SELECT @n = geom FROM POL_adm2 WHERE NAME_2 = 'Warsaw'
SELECT @l = @m.ShortestLineTo(@n)
SELECT @m.STDistance(@n) / 1000 AS "Dist (km)"

SELECT NAME_1 AS Name_, geom FROM POL_adm1 WHERE NAME_1 = 'Masovian'
UNION ALL
SELECT 'line', @l

UNION ALL
SELECT NAME_2 AS Name_, geom FROM POL_adm2 WHERE geom.STIntersects(@l) = 1


-- Show neighbouring districts
DECLARE @e geography
SELECT @e = geom FROM POL_adm2 WHERE NAME_2 = 'Warsaw'
SELECT id, NAME_1 AS Name_, geom FROM POL_adm1 WHERE NAME_1 = 'Masovian'
UNION ALL
SELECT id, NAME_2 AS Name_, geom FROM POL_adm2
