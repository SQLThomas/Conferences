...
mt <- data.frame(
  parties = factor(c("Indep2", "PN", "Indep1", "PL"),
                   levels = c("Indep2", "PN", "Indep1", "PL")),
  seats   = c(2, 28, 1, 36),
  colors  = c("lightblue", "blue", "grey", "red"),
  stringsAsFactors = FALSE)

mt
ggplot(mt) + 
  geom_parliament(aes(seats = seats, fill = parties), color = "black") + 
  scale_fill_manual(values = mt$colors, labels = mt$parties) +
  coord_fixed() + labs(title = 'Kamra Tad-Deputati', caption = 'Source: Wikipedia') +
